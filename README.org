...
Hence, =zref-check= does not automate the format of your cross-reference text is
typeset, it just provides instead a warning so that the problem can be
identified (and fixed) without having to rely on burdensome and error prone
manual proof-reading.

For more details, check the [[zref-check.pdf][Documentation]].
